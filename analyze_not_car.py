...
def analyze_skins():
    # Updated to look in not_car folder specifically
    examples_dir = Path("examples/not_car")
    zips = list(examples_dir.glob("*.zip"))
    
    print(f"Found {len(zips)} 'Not Car' skins to analyze.\n")
    ...
    for z in zips:
        ...
        try:
            with zipfile.ZipFile(z, 'r') as zf:
                ...
                if audio:
                    print(f"  Audio ({len(audio)}): {len(audio)} files (e.g. {audio[0] if audio else ''})")
                if others:
